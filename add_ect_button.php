...
if(!empty($_POST))
	{
		...
		$s='<form method="post" action="/cart.php">';
			$s.='<input type="hidden" name="id" value="'.esc_sql($_POST['prod_id']).'" />';
			$s.='<input type="hidden" name="mode" value="add" />';
			if(esc_sql($_POST['btn_type'])=='img')
				$s.='<input type="image" src="/images/buy.gif" border="0" />';
			elseif(esc_sql($_POST['btn_type'])=='txt')
			{	
				$cl='';
				$vl='Add to cart';
				if(!empty(esc_sql($_POST['class_name'])))
					$cl="class='".esc_sql($_POST['class_name'])."'";
				if(!empty(esc_sql($_POST['valn'])))
					$vl=$_POST['valn'];
				$s.='<input type="submit" '.$cl.' name="'.$vl.'" value="'.$vl.'">';
			}
		$s.='</form>';
		update_option('shortcode_ec_'.esc_sql($_POST['prod_id']),addslashes($s));
